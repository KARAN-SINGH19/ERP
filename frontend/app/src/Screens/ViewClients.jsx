...
const ViewClients = () => {
    ...
    async function fetchClients() {
        try {
            ...
            const response = await axios.get("http://localhost:6000/api/v1/getClients", {
                headers: {
    ...
    const deleteClient = async (id) => {
        ...
        if (confirmDelete.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:6000/api/v1/deleteClient/${id}`);
                if (response.data.success) {
                    Swal.fire({
                        title: "Deleted!",
    ...
    const handleSearchClick = async () => {
        try {
            const response = await axios.get('http://localhost:6000/api/v1/searchClients', {
                params: {
