...
const ViewPositions = () => {
    ...
    async function fetchPositions() {
        try {
            ...
            const response = await axios.get("http://localhost:6000/api/v1/getPositions", {
                headers: {
    ...
    const deletePosition = async (id) => {
        ...
        if (confirmDelete.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:6000/api/v1/deletePosition/${id}`);
                if (response.data.success) {
                    Swal.fire({
                        title: "Deleted!",
    ...
    const handleSearchClick = async () => {
        try {
            const response = await axios.get('http://localhost:6000/api/v1/searchPositions', {
                params: {
    ...
    const handleStatusChange = async (id, e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.put(`http://localhost:6000/api/v1/updateStatus/${id}`, { status: newStatus }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
