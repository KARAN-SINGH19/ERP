...
const ViewCandidates = () => {
    ...
    async function fetchCandidates() {
        try {
            ...
            const response = await axios.get("http://localhost:5000/api/v1/getCandidates", {
                headers: {
    ...
    const deleteUser = async (id) => {
        ...
        if (alert.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:5000/api/v1/deleteCandidate/${id}`);
                if (response.data.success) {
                    Swal.fire({
                        title: "Deleted!",
    ...
    const handleSearchClick = async () => {
        try {
            const response = await axios.get('http://localhost:5000/api/v1/searchCandidates', {
                params: {
                                            ...
                                            <tr key={candidate._id}>
                                                ...
                                                <td className="actions">
                                                    {candidate.resume && candidate.resume.fileId ? (
                                                        <Button
                                                            onClick={() => handleResumeDownload(`http://localhost:5000/api/v1/downloadResume/${candidate.resume.fileId}`, candidate.resume.filename)}
                                                            variant="primary"
