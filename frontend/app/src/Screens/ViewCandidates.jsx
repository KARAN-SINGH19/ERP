...
const ViewCandidates = () => {
    ...
    async function fetchCandidates() {
        try {
            ...
            const response = await axios.get("http://82.112.236.55:5000/api/v1/getCandidates", {
                headers: {
    ...
    const deleteUser = async (id) => {
        ...
        if (alert.isConfirmed) {
            try {
                const response = await axios.delete(`http://82.112.236.55:5000/api/v1/deleteCandidate/${id}`);
                if (response.data.success) {
                    Swal.fire({
                        title: "Deleted!",
    ...
    const handleSearchClick = async () => {
        try {
            const response = await axios.get('http://82.112.236.55:5000/api/v1/searchCandidates', {
                params: {
                                            ...
                                            <tr key={candidate._id}>
                                                ...
                                                <td className="actions">
                                                    {candidate.resume && candidate.resume.fileId ? (
                                                        <Button
                                                            onClick={() => handleResumeDownload(`http://82.112.236.55:5000/api/v1/downloadResume/${candidate.resume.fileId}`, candidate.resume.filename)}
                                                            variant="primary"
